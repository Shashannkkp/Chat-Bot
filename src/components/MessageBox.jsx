...
const MessageBox = forwardRef(({ loading, setLoading, callApp, allowImageAttachment }, ref) => {
    const { t } = useTranslation();
    const textValue = useRef('');
    const [isTextEmpty, setIsTextEmpty] = useState(true);
    const textAreaRef = useRef(null);
    const resizeTimeoutRef = useRef(null);
    const [fileDataRef, setFileDataRef] = useState([]);
    ...
    function clearValueAndUndoHistory(textAreaRef) {
        setFileDataRef([]);
        setTextValue('');
        setTextAreaValue('');
    }
    const insertTextAtCursorPosition = (textToInsert) => {
        if (textAreaRef.current) {
            ...
            const newTextValue = text.substring(0, startPos) +
                textToInsert +
    ...
    const handleRemoveFileData = (index, fileRef) => {
        setFileDataRef(fileDataRef.filter((_, i) => i !== index));
    };
    return (_jsx("div", { style: { position: "sticky" }, className: "absolute bottom-0 left-0 w-full border-t md:border-t-0 dark:border-white/20 md:border-transparent md:dark:border-transparent bg-white dark:bg-gray-900 md:bg-transparent! pt-2", children: _jsx("form", { onSubmit: handleSubmit, className: "stretch mx-2 flex flex-row gap-3 last:mb-2 md:mx-4 md:last:mb-6 lg:mx-auto md:max-w-2xl lg:max-w-3xl xl:max-w-4xl 2xl:max-w-5xl 3xl:max-w-6xl 4xl:max-w7xl", children: _jsxs("div", { id: "message-box-border", style: { borderRadius: "1rem" }, className: "relative flex flex-col h-full flex-1 w-full py-2 grow md:py-3 bg-white dark:bg-gray-850\r\n               dark:text-white dark:bg-gray-850 border border-black/10 dark:border-gray-900/50\r\n               focus-within:border-black/30 dark:focus-within:border-gray-500/50", children: [fileDataRef.length > 0 && (_jsx("div", { className: "w-full", children: _jsx(FileDataPreview, { fileDataRef: fileDataRef, removeFileData: handleRemoveFileData, allowImageAttachment: allowImageAttachment == 'yes' }) })), _jsxs("div", { className: "flex items-center w-full relative space-x-2", children: [_jsx("div", { className: "flex items-center justify-start", children: _jsx("button", { onClick: (e) => handleAttachment(e), className: "p-1 relative z-10", children: _jsx(PaperClipIcon, { className: "h-6 w-6" }) }) }), _jsx("div", { className: "flex items-center ", style: { flexShrink: 0, minWidth: 'fit-content' } }), _jsx("textarea", { id: "sendMessageInput", name: "message", tabIndex: 0, ref: textAreaRef, rows: 1, className: "flex-auto m-0 resize-none border-0 bg-transparent px-2 py-2 focus:ring-0 focus-visible:ring-0 outline-hidden shadow-none dark:bg-transparent", placeholder: t('send-a-message'), onKeyDown: checkForSpecialKey, onChange: handleTextChange, onPaste: handlePaste, style: { minWidth: 0 } }), _jsx("div", { className: "flex items-center justify-end", children: loading ? (_jsx(Tooltip, { title: t('cancel-output'), side: "top", sideOffset: 0, children: _jsx("button", { onClick: (e) => handleCancel(e), className: "p-1", children: _jsx(StopCircleIcon, { className: "h-6 w-6" }) }) })) : (_jsx(SubmitButton, { disabled: isTextEmpty || loading, loading: loading })) })] })] }) }) }));
});
