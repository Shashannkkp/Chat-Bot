...
function getFirstValidString(...args) {
...
const MainPage = ({ className, isSidebarCollapsed, toggleSidebarCollapse }) => {
    const { userSettings, setUserSettings } = useContext(UserContext);
    const { t } = useTranslation();
    const [chatSettings, setChatSettings] = useState(undefined);
    ...
    const chatSettingsListener = (data) => {
    ...
    const fetchAndSetChatSettings = async (gid) => {
        try {
        ...
        if (id && id.length > 0) {
            ...
            ConversationService.getConversationById(n)
                ...
                if (conversation) {
                    ...
                    clearInputArea();
                    ConversationService.getChatMessages(conversation).then((msgs) => {
                        if (msgs.length === 0) {
    ...
    function startConversation(message, fileDataRef) {
        const id = Date.now();
    ...
    const callApp = (message, fileDataRef) => {
        if (!conversation) {
    ...
    const addMessage = (role, messageType, message, fileDataRef, callback) => {
        let content = message;
    ...
    function getEffectiveChatSettings() {
    ...
    function sendMessage(updatedMessages) {
        ...
        ChatService.sendMessage(messages, effectiveSettings.model)
            .then((responseText) => {
            ...
            if (err instanceof CustomError) {
                ...
                NotificationService.handleUnexpectedError(err, 'Failed to send message to Gemini.');
            }
        })
            .finally(() => {
            setLoading(false);
    ...
    function handleStreamedResponse(content, fileDataRef) {
        setMessages(prevMessages => {
            let isNew = false;
            try {
    ...
    const handleUserScroll = (isAtBottom) => {
    ...
    const handleImportConversation = async () => {
        try {
            ...
            console.error(error);
            NotificationService.handleUnexpectedError(error, "Failed to import conversation");
        }
    };
    // ---------------------------------
    ...
    const createButton = () => {
        ...
        root.render(_jsx(ArrowUturnDownIcon, {}));
    ...
    // ---------------------------------
    return (_jsx("div", { className: `${className} overflow-hidden w-full h-full relative flex z-0 dark:bg-gray-900`, children: _jsx("div", { className: "flex flex-col items-stretch w-full h-full", children: _jsxs("main", { className: "relative h-full transition-width flex flex-col overflow-hidden items-stretch flex-1", onMouseUp: handleMouseUp, children: [gid ? (_jsx("div", { className: `inline-block absolute top-0 left-0 z-50 ${isSidebarCollapsed ? 'sidebar-collapsed-margin' : 'sidebar-expanded-margin'}`, children: _jsx(ChatSettingDropdownMenu, { chatSetting: chatSettings }) })) : null, _jsxs("div", { className: "absolute top-2 right-2 z-50 flex gap-2", children: [_jsx("button", { onClick: handleImportConversation, className: "px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700", children: t('Import Chat') }), conversation && (_jsx("button", { onClick: handleExportConversation, className: "px-4 py-2 mr-2 bg-blue-500 text-white rounded-md hover:bg-blue-600", children: t('Export Chat') }))] }), !conversation && chatSettings ? (_jsx(CustomChatSplash, { className: " -translate-y-[10%] ", chatSettings: chatSettings })) : null, _jsx(Chat, { chatBlocks: messages, onChatScroll: handleUserScroll, conversation: conversation, model: model?.id || DEFAULT_MODEL, onModelChange: handleModelChange, allowAutoScroll: allowAutoScroll, loading: loading }), showScrollButton && (_jsx("div", { className: "absolute bottom-0 left-1/2 transform -translate-x-1/2 mb-10 z-10", children: _jsx(ScrollToBottomButton, { onClick: scrollToBottom }) })), _jsx(MessageBox, { ref: messageBoxRef, callApp: callApp, loading: loading, setLoading: setLoading, allowImageAttachment: model === null || model?.image_support || false
                            ? 'yes'
                            : !conversation
                                ? 'warn'
                                : 'no' })] }) }) }));
