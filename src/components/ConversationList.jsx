...
import ConversationService, { conversationsEmitter } from "../service/ConversationService";
...
const ConversationList = () => {
    ...
    const getHeaderFromTimestamp = (timestamp) => {
    ...
    const insertTimeMarkers = (conversations) => {
        let lastHeader = "";
        const withMarkers = [];
    ...
    return (_jsxs("div", { className: "conversation-list-container", children: [_jsxs("div", { id: "conversation-search", className: "flex flex-row items-center mb-2 relative", children: [_jsx("input", { id: "searchInput", className: "grow rounded-md border dark:text-gray-100 dark:bg-gray-850 dark:border-white/20 px-2 py-1", type: "text", autoComplete: "off", placeholder: t('search'), value: searchInputValue, onFocus: () => setShowSearchOptions(true), onBlur: () => setTimeout(() => setShowSearchOptions(false), 200), onChange: (e) => setSearchInputValue(e.target.value), onKeyPress: (e) => {
                            if (e.key === 'Enter') {
                                handleSearch(searchInputValue);
                                setShowSearchOptions(false);
                            }
                        } }), _jsx("button", { className: "ml-2 rounded-md border dark:border-white/20 p-1", onClick: () => {
                            handleSearch(searchInputValue);
                            setShowSearchOptions(false);
                        }, children: _jsx(MagnifyingGlassIcon, { style: { color: userSettings.theme === 'dark' ? '#FFFFFF' : '#000000' }, ...iconProps }) })] }), _jsx("div", { id: "conversation-list", ref: scrollContainerRef, className: "flex-col flex-1 transition-opacity duration-500 -mr-2 pr-2 overflow-y-auto", children: _jsx("div", { className: "flex flex-col gap-2 pb-2 dark:text-gray-100 text-gray-800 text-sm", children: _jsx("div", { className: "relative overflow-x-hidden", style: { height: "auto", opacity: 1 }, children: _jsx("ol", { children: conversationsWithMarkers.map((convo, index) => {
                                if ("marker" in convo) {
                                    return (_jsx("li", { className: "sticky top-0 z-16", children: _jsx("h3", { className: "h-9 pb-2 pt-3 px-3 text-xs text-gray-500 font-medium text-ellipsis overflow-hidden bg-gray-50 dark:bg-gray-900", children: convo.title }) }, `marker-${index}`));
                                }
                                else {
                                    return (_jsx(ConversationListItemMemo, { convo: convo, isSelected: selectedId === convo.id, loadConversations: loadConversations, setSelectedId: setSelectedId }, convo.id));
                                }
                            }) }) }) }) })] }));
};
